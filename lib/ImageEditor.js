  ...
  $.fn.ImageEditor = function(options) {
    ...
    ImageEditor.prototype = {
      ...
      _placeImage: function(image) {
        ...
        // resize image
		  console.log(image.size);
		if(image.size === 'basket'){
			$img.css('width', '60px');
			$img.css('height', '37px');
			image.width = 60;
			image.height = 37;
//			$imgWrapper.css('left', '150px');
//			$imgWrapper.css('top', '45px');
			$imgWrapper.css('transform','translate(127px,69px)');
			//$imgWrapper.style.webkitTransform = "translate(150px,45px)";
		}else if(image.size === 'bike'){
			$img.css('width', '180px');
        	$img.css('height', '120px');
			image.width = 180;
			image.height = 120;
			$imgWrapper.css('left', '135px');
        	$imgWrapper.css('top', '63px');
		}else{
      ...
      _preProcessImageUrl: function(url) {
        ...
        //console.log('pre process = ', defaultObj.size);
        return defaultObj;
      },
      ...
      addImage: function(url, select, index, size) {
		  url = this._preProcessImageUrl(url,index,size);
        if(select === undefined)
          select = true;
        ...
        var $img = $('<img />'),
            ...
            image = {
              ...
              $imgWrapper: null,
              $removeIcon: null,
              order: url.index || this.images.length + 1, // starts from 1
			  size:url.size,
...
//            clickToSelect: url.clickToSelect,
      ...
      mergeImage: function() {
		...
		this.images.splice(this.images.length,0,this.images[2]);
		this.images[1].centerPoint.x = 234;
		this.images[1].centerPoint.y = 128;
		this.images[2].centerPoint.x = 165;
		this.images[2].centerPoint.y = 100;
